...
namespace AspireAIAgentsCarResales.Web.Services.MultiAgents
{
    public class SemanticKernelService : ISemanticKernelService, IDisposable
    {
        ...
        public async Task<List<ChatMessageContent>> GetDemoResponse(string userInput)
        {
            ...
            const string firststage = "RegionalDemand";
            const string secondstage = "EconomicImpact";
            const string thirdstage = "CompetitiveMarket";
            const string fourthstage = "FinalReview";

            // Build the kernel  
            var kernel = _semanticKernel;

            ChatCompletionAgent RegionalDemandAgent =
                new()
                {
                    Instructions = $"""{SystemPromptFactory.GetAgentPrompts(AgentType.RegionalDemand)}""",
                    Name = firststage,
                    Kernel = kernel
                };

            ChatCompletionAgent EconomicImpactAgent =
                new()
                {
                    Instructions = $"""{SystemPromptFactory.GetAgentPrompts(AgentType.EconomicImpact)}""",
                    Name = secondstage,
                    Kernel = kernel
                };

            ChatCompletionAgent CompetitiveMarketnAgent =
                new()
                {
                    Instructions = $"""{SystemPromptFactory.GetAgentPrompts(AgentType.CompetitiveMarket)}""",
                    Name = thirdstage,
                    Kernel = kernel
                };


            ChatCompletionAgent FinalReviewAgent =
                new()
                {
                    Instructions = $"""{SystemPromptFactory.GetAgentPrompts(AgentType.FinalReview)}""",
                    Name = fourthstage,
                    Kernel = kernel
                };
...
#pragma warning disable SKEXP0110 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.  
           AgentGroupChat chat =
                new(RegionalDemandAgent, EconomicImpactAgent, CompetitiveMarketnAgent, FinalReviewAgent)
                {
                    ExecutionSettings =
                        new()
                        {
